...
function notifyTelegram(prUrl: string, docPath: string): void {
  ...
  const msg = encodeURIComponent(
    `📄 Doc auto-update (review required)\n\nFichier : ${docPath}\nTier : ${tierLabel(2)}\n\nPR: ${prUrl}\n\nRevue humaine nécessaire avant merge.`,
  );
  try {
    execSync(
      `curl -s "https://api.telegram.org/bot${token}/sendMessage?chat_id=${userId}&text=${msg}&parse_mode=Markdown"`,
      { stdio: "pipe" },
    );
...
async function main(): Promise<void> {
  ...
  try {
    prUrl = run(
      `gh pr create --title "${prTitle}" --body "${prBody}" --base master --head ${branchName}`,
      { silent: true },
    ).trim();
  ...
  if (plan.tier1.length > 0 && plan.tier2.length === 0) {
    try {
      run(`gh pr merge --auto --merge "${prUrl}"`, { silent: true });
