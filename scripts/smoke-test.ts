...
async function checkPM2(): Promise<SmokeResult> {
  ...
  try {
    ...
    const apps = JSON.parse(output) as Array<{ name: string; pm2_env: { status: string; restart_time: number } }>;
    ...
    if (issues.length > 0) {
      return { check: "PM2", status: "fail", detail: issues.join(", "), durationMs: Date.now() - start };
    }
...
async function checkDashboard(): Promise<SmokeResult> {
  ...
  try {
    ...
    if (!res.ok) {
      return { check: "Dashboard", status: "fail", detail: `HTTP ${res.status}`, durationMs: Date.now() - start };
    }
    ...
    return { check: "Dashboard", status: "fail", detail: String(err), durationMs: Date.now() - start };
  }
}
...
async function checkSupabase(): Promise<SmokeResult> {
  ...
  if (!url || !key) {
    return { check: "Supabase", status: "warning", detail: "SUPABASE_URL or SUPABASE_ANON_KEY not set", durationMs: Date.now() - start };
  }
  ...
  try {
    ...
    if (insertErr) {
      return { check: "Supabase", status: "fail", detail: `Insert failed: ${insertErr.message}`, durationMs: Date.now() - start };
    }
    ...
    return { check: "Supabase", status: "pass", detail: "insert+delete OK", durationMs: Date.now() - start };
  } catch (err) {
    return { check: "Supabase", status: "fail", detail: String(err), durationMs: Date.now() - start };
  }
}
...
async function checkClaudeCLI(): Promise<SmokeResult> {
  ...
  try {
    ...
    const proc = spawn([claudePath, "-p", "Reply with exactly: SMOKE_OK", "--output-format", "text", "--dangerously-skip-permissions"], {
      stdout: "pipe",
      stderr: "pipe",
      timeout: 15000,
    });
    ...
    if (exitCode !== 0) {
      return { check: "Claude CLI", status: "fail", detail: `exit code ${exitCode}`, durationMs: Date.now() - start };
    }

    if (!stdout.includes("SMOKE_OK")) {
      return { check: "Claude CLI", status: "warning", detail: "Response didn't contain SMOKE_OK", durationMs: Date.now() - start };
    }
    ...
    if (msg.includes("Timeout") || msg.includes("rate")) {
      return { check: "Claude CLI", status: "warning", detail: msg, durationMs: Date.now() - start };
    }
...
async function checkTelegram(): Promise<SmokeResult> {
  ...
  if (!token || !userId) {
    return { check: "Telegram", status: "warning", detail: "TELEGRAM_BOT_TOKEN or TELEGRAM_USER_ID not set", durationMs: Date.now() - start };
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return { check: "Telegram", status: "warning", detail: `HTTP ${res.status}`, durationMs: Date.now() - start };
    }

    return { check: "Telegram", status: "pass", detail: "message sent", durationMs: Date.now() - start };
  } catch (err) {
    return { check: "Telegram", status: "warning", detail: String(err), durationMs: Date.now() - start };
  }
}
